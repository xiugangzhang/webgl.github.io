#### 高光贴图的使用
<p><iframe style="width: 100%; height: 520px; text-align:center;" src="/nehe_webgl/lessons/NeHeWebGL23.html" frameborder="0" width="500" height="500"></iframe></p>
...
<script id="per-fragment-lighting-fs" type="x-shader/x-fragment">
    ...
    void main(void) {
        ...
        if (!uUseLighting) {
            ...
            vec3 normal = normalize(vTransformedNormal);                    //这个是从顶点着色器中传过来的变量

            float specularLightWeighting = 0.0;                     //用来指定specular光照的权重； 如果不进行高光反射，这个变量将被赋值为0
            float shininess = 32.0;                                 //初始的反光度为32
            if (uUseSpecularMap) {
                shininess = texture2D(uSpecularMapSampler, vec2(vTextureCoord.s, vTextureCoord.t)).r * 255.0;       //取出高光纹理贴图的R通道数据
            }
            if (shininess < 255.0) {            //这里只对于小于255的反光度进行实际的高光计算
                vec3 eyeDirection = normalize(-vPosition.xyz);
                vec3 reflectionDirection = reflect(-lightDirection, normal);

                specularLightWeighting = pow(max(dot(reflectionDirection, eyeDirection), 0.0), shininess);  //这里传入了shininess这个变量，这里的反光度是从高光贴图中去取出来的哈
            }
        ...
        vec4 fragmentColor;
        if (uUseColorMap) {             //物体的颜色是从颜色纹理中取出来的
            fragmentColor = texture2D(uColorMapSampler, vec2(vTextureCoord.s, vTextureCoord.t));
        } else {
            fragmentColor = vec4(1.0, 1.0, 1.0, 1.0);       //不使用颜色纹理的话，默认的颜色为纯白色
        }
...
<script type="text/javascript">
    ...
    //这里定义了两张纹理图片
    var earthColorMapTexture;               //这个纹理图片用来指定颜色信息
    var earthSpecularMapTexture;            //这个纹理图片用来指定反光度
    ...
    function initTextures() {
        ...
        earthColorMapTexture.image.src = "../resources/earth/earth.jpg";
        ...
        earthSpecularMapTexture.image.src = "../resources/earth/earth-specular.gif";
    }
    ...
    function drawScene() {
        ...
        gl.uniform1i(shaderProgram.colorMapSamplerUniform, 0);          //这里使用了颜色贴图

        //我在这里使用了1号纹理单元
        gl.activeTexture(gl.TEXTURE1);
        gl.bindTexture(gl.TEXTURE_2D, earthSpecularMapTexture);
        gl.uniform1i(shaderProgram.specularMapSamplerUniform, 1);       //这里开启了高光贴图
